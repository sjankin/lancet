...
intersection_df <- tidytext::tidy(intersection_dfm) %>% dplyr::rename(docid = document) %>% 
  dplyr::mutate(line = stringr::str_extract(docid, stringr::regex("\\.L\\d*")),
                ...
                document_id = stringr::str_extract(docid, stringr::regex("\\d{4}\\_[A-Z]{3}\\_\\d*")))


intersection_covid <- 
  tibble::tibble(
    year = 2020,
    intersection_df %>% dplyr::filter(year == 2020) %>% dplyr::summarize(total_docs = dplyr::n_distinct(document_id)),
...
gender_intersection_df <- quanteda::dfm_lookup(intersection_dfm, dictionary = gender_dict, valuetype= "fixed") %>% convert(., "tm") %>%
  tidytext::tidy(.) %>% dplyr::mutate(line = stringr::str_extract(document, stringr::regex("\\.L\\d*")),
                                      year = stringr::str_extract(document, stringr::regex("\\d{4}")),
                                      document_id = stringr::str_extract(document, stringr::regex("\\d{4}\\_[A-Z]{3}\\_\\d*")))

intersection_gender <- tibble::tibble(
  intersection_df %>% dplyr::filter(year >= 2011) %>% group_by(year) %>% dplyr::summarize(total_docs = dplyr::n_distinct(document_id)),
  gender_intersection_df %>% dplyr::filter(year >= 2011) %>% 
    group_by(year) %>% dplyr::summarize(hits = sum(count),documents = n_distinct(document)) %>%
    dplyr::select(-year)) %>% 
...
# join to text info
total_counts <- left_join(ungc_df, counts_df, by = c("doc_id" = "docid")) %>% left_join(., country_lists)
total_counts <- arrange(total_counts, Country, Year, Sector, Participant)

# Data preparation
climate_texts <- total_counts %>% filter(climate_count>0) %>% group_by(Sector, Year) %>% tally(name = "climate_texts")
health_texts <- total_counts %>% filter(health_count>0) %>% group_by(Sector, Year) %>% tally(name = "health_texts")
intersection_texts <- total_counts %>% filter(intersection_count>0) %>% group_by(Sector, Year) %>% tally(name = "intersection_texts")

total_texts <- ungc_df %>% group_by(Sector, Year) %>% tally(name = "total_texts") %>%
