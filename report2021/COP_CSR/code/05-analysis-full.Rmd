...
### Proportion of companies by industry 
...
Sub-setting corpus by size (~1000 documents)
```{r}
ungc_files %>%
  group_by(Sector) %>%
  count()
...
```{r}
group1 <- c("Aerospace&Defense", "AlternativeEnergy", "Automobiles&Parts", "Banks", "Beverages", "Chemicals", "Construction&Materials", "Diversified", "Electricity", "Electronic&ElectricalEqu...", "EquityInvestmentInstruments", "FinancialServices")
group2 <- c("FixedLineTelecommunications", "Food&DrugRetailers", "FoodProducers", "Forestry&Paper", "Gas,Water&Multiutilities", "GeneralIndustrials", "GeneralRetailers", "HealthCareEquipment&Ser...", "HouseholdGoods&HomeCons...", "IndustrialEngineering", "IndustrialGoods&Services", "IndustrialMetals&Mining", "IndustrialTransportation", "LeisureGoods", "LifeInsurance", "Media", "Mining", "MobileTelecommunications", "NonequityInvestmentInstru...	", "NonlifeInsurance")
group3 <- c("Oil&GasProducers", "OilEquipment,Services&D...", "PersonalGoods", "Pharmaceuticals&Biotechno...", "RealEstate", "RealEstateInvestment&Se...", "RealEstateInvestmentTrusts", "Retail", "Software&ComputerServices", "SupportServices", "TechnologyHardware&Equip...", "Travel&Leisure")
```

```{r}
ungc_g1 <- ungc_files %>%
  filter(Sector %in% group1)

ungc_g2 <- ungc_files %>%
  filter(Sector %in% group2)

ungc_g3 <- ungc_files %>%
  filter(Sector %in% group3)
```
...
#ungc_corpus <- corpus(ungc_g1, text_field = "text")
#ungc_corpus <- corpus(ungc_g2, text_field = "text") 
ungc_corpus <- corpus(ungc_g3, text_field = "text") 
```
...
#readr::write_csv(corp_summary, "corp_summary_2019.csv")
```
...
```{r}
readr::write_csv(total_counts, "total_counts_g3.csv")
```

Per Sector by Year
```{r}
# import files
total_counts_g1 <- readr::read_csv("total_counts_g1.csv")
total_counts_g2 <- readr::read_csv("total_counts_g2.csv")
total_counts_g3 <- readr::read_csv("total_counts_g3.csv")


total_counts <- rbind(total_counts_g1, total_counts_g2, total_counts_g3)
readr::write_csv(total_counts, "total_counts_2019.csv")
```

```{r}
# Data preparation
climate_texts <- total_counts %>% filter(climate_count>0) %>% group_by(sector) %>% tally(name = "climate_texts")
health_texts <- total_counts %>% filter(health_count>0) %>% group_by(sector) %>% tally(name = "health_texts")
intersection_texts <- total_counts %>% filter(intersection_count>0) %>% group_by(sector) %>% tally(name = "intersection_texts")

total_texts <- ungc_files %>% group_by(Sector) %>% tally(name = "total_texts")

proportions <- left_join(total_texts, health_texts, by = c("Sector"="sector")) %>% 
  left_join(., climate_texts, by = c("Sector"="sector")) %>% 
  left_join(., intersection_texts, by = c("Sector"="sector")) %>% 
...
#readr::write_csv(proportions, "proportion_results_bysector_2019.csv")
```

```{r}
proportions_t <- proportions %>%
  select("Sector", "percent_int", "percent_CC", "percent_PH") %>%
  rename("intersection" = "percent_int", 
         "climate" = "percent_CC",
         "health" = "percent_PH") %>%
  filter(intersection > 0 | health > 0 | climate > 0) %>%
  gather(`intersection`, `climate`, `health`, key="topic", value="percentage")
  
```


```{r}
ggplot(proportions_t, aes(x=Sector, y=percentage, fill=topic)) +
  geom_col(position=position_dodge(), width=.75) +
  #geom_col(aes(y=percent_int), fill="red", alpha = 0.9) +
  #geom_col(aes(y=percent_CC), fill="darkgreen", alpha = 0.9) +
  #geom_col(aes(y=percent_PH), fill="blue",alpha = 0.9) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
  ylab("Proportion of companies, %")  +
...
ggsave("proportions_by_sector_2019.pdf", height=6, width=12)
```


```{r}
ggplot(proportions, aes(x=Sector))
```
