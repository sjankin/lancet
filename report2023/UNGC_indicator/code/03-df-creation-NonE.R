...
# bind dfs from list
csr_text_df <- dplyr::bind_rows(txt_list) %>% dplyr::mutate(Id = as.character(Id)) %>% dplyr::left_join(csr_non_english, .)
sum(is.na(csr_text_df$text))
save(csr_text_df, file = "data/csr_text_df_nonEnglish.Rdata")
