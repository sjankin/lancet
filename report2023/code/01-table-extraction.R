...
# check country name
sort(unique(csr_cop_submissions$Country))
...
# iso code matching - exclusion of 2023+
csr_table <- left_join(csr_cop_submissions, iso_codes[c('Country', 'code')], by = c("Country")) %>%
  dplyr::filter(Year != "2023")
sort(unique(csr_table$Country))
# save intermediate step
save(csr_table, file = "data/csr_table_step1.Rdata")

# if starting with this step then run from this. otherwise start from top
load("data/csr_table_step1.Rdata")
...
# save table -----  containing all available info for meta data 
